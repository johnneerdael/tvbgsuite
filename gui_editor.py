...
def generate_preview_server():
    ...
    engine.draw_custom_text_and_provider_logo("Preview Generated by Engine", "static/provider_logos/tmdblogo.png")
...
def fetch_trakt_list(config):
    t = config.get('trakt', {})
    catalog_ids = t.get('catalogs', {}).get('enabled', ['trakt_trending_movies', 'trakt_trending_shows'])
    return TraktClient(t).fetch_catalog_items(catalog_ids, limit=20)
...
def get_media_list():
    ...
    providers = request.args.get('providers', 'tmdb').split(',')
...
def search_media():
    ...
    if not api_key: return jsonify([])

    try:
        r = requests.get(
            "https://api.themoviedb.org/3/search/multi",
            params={"api_key": api_key, "query": query, "language": tmdb.get("language", "en-US")},
            timeout=5,
        )
        r.raise_for_status()
        results = []
        for item in r.json().get('results', [])[:10]:
            ...
            results.append({"Id": f"tmdb-{media_type}-{item['id']}", "Name": title, "ProductionYear": year})
...
def enrich_media_metadata(metadata, media_type, config):
    routed = metadata
    if media_type == "tv" and config.get("metadata", {}).get("tv_provider") == "tvdb":
        ...
        routed = merge_metadata(metadata, fetch_tvdb_details(tvdb_id, config)) if tvdb_id else choose_tv_metadata(metadata, media_type, config)
    else:
    ...
    if provider == "trakt":
        ...
        if len(parts) >= 2:
            ...
            if details:
                ...
                return jsonify(enrich_media_metadata(details, mtype, config))
        ...
        details = fetch_tvdb_details(actual_id, config)
        if details:
            return jsonify(apply_configured_rating(details, config))
...
def test_trakt():
    ...
    try:
        ...
        r = requests.get("https://api.trakt.tv/movies/trending?limit=1", headers=headers, timeout=5)
        r.raise_for_status()
        return jsonify({"status": "success", "message": "Connected to Trakt"})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def trakt_catalogs():
    ...
    return jsonify({
        "built_in": TRAKT_BUILT_IN_CATALOG_OPTIONS,
        "popular_lists": client.fetch_popular_lists(),
    })
